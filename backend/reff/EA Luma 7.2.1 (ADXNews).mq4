...
//|                                                         ADX NEWS |
//+------------------------------------------------------------------+
#property copyright "Luma 7.2.1 - ADX NEWS"
#property link      ""
...
int MagicNumber;
double slippage = 3;
string EAName = "Luma 7.2.1 ADX NEWS";
...
void CreateDashboard() {
   ...
   ObjectSetString(0, "VENUS_TITLE", OBJPROP_TEXT, "Luma EA v7.2.1 ADX NEWS");
...
bool CheckADXSpikeEntry(int &signalDirection) {
   ...
   if (adxSpikeLastSignalTime > 0) {
      ...
      if (elapsedMinutes < ADXSpikeCooldownMinutes) {
         return false;
      }
   }
   ...
   if (IsADXSpike(dmiDir, isReversalSignal) && dmiDir != -1) {
      ...
      if (smcConfirmOK) {
         // ===== KONFIRMASI HIGHER TIMEFRAME (dengan info reversal mode) =====
